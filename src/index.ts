...
if (currentTheme) {
    document.addEventListener("DOMContentLoaded", () => dispatch(actionSetTheme(setTheme[`${currentTheme}`])));
...
if (alias === "activity" || alias === "vote") {
    window.addEventListener(
        "orientationchange",
        function () {
            setTimeout(() => (document.body.innerHTML = window.renderTemplate(alias, data)), 20);
        },
        false
    );
}
